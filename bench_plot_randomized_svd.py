...
## 待测试数据集，minst-driginal和ala数据集缺失，rcv1无法正常下载
datasets = ['low rank matrix', 'lfw_people', 'olivetti_faces', '20newsgroups',
             'CIFAR', 'SVHN', 'uncorrelated matrix']

big_sparse_datasets = ['big sparse matrix']


def unpickle(file_name):
    with open(file_name, 'rb') as fo:
        return pickle.load(fo)["data"]
...
## newsgroup 数据集调整为11314*10000，避免内存溢出
def get_data(dataset_name):
    ...
    if dataset_name == 'lfw_people':
        ...
        X = fetch_20newsgroups_vectorized().data[:, :10000]
